...
3. 取最底部 8 行（离车最近，约车前 6~7cm），分 4 个区间
   `bit3=[50,65) bit2=[65,80) bit1=[80,95) bit0=[95,110)`，
   区间内黑像素（灰度 < 阈值，默认 110 可实时调）占比达标才判“有线”：
   **中间两路 ≥5%、两侧路 ≥10%**，
   与四路红外逻辑一一对应，16 状态查表无需改动。另外在底部 8 行之上
   再取 24 行作为“远带预览”：
   底部无线但远带有线时进入弯道模式，按远带方向提前转向（用满转向量），
   改善锐角弯；日志 `LINE ... near=0b.. far=0b.. bits=0b..` 可观察两带。
...
2. `ESP-IDF: Build App`，然后 `ESP-IDF: Flash your Project`（UART，串口 115200）。
