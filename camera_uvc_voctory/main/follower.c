...
#define AVOID_CLEAR_CONFIRM_MS   250   /* 障碍清除后再多左移 250ms */
...
#define PASS_FORWARD_MS    1400   /* 避障直行越障时长 */
#define PASS_LF_SPEED      114
#define PASS_RF_SPEED      126
#define STRAFE_BACK_MS        1400   /* 直行越障后右移最大时长 */
#define STRAFE_BACK_MIN_MS    400    /* 右移至少时长，之后才允许因看到黑线提前停止 */
#define REACQUIRE_PAUSE_MS 120

/* 避障结束后的收尾：再看到黑线 -> 原地转对正 -> 直行后停止 */
#define POST_ALIGN_STRAIGHT_MS  700
...
#define BALL_DETECT_PIXELS        12     /* 达到该红点数才算“看到球”（red-ball-push 新版） */
...
#define BALL_SEARCH_STEP_MS       100    /* 每次给动力时长：找球/首次对准都用 100ms，过冲后 100→50→25→20 */
#define BALL_ALIGN_STEP_MIN_MS    20     /* 微调档最短脉冲（red-ball-push 新版） */
...
#define BALL_TURN_SPEED           120    /* 转向脉冲功率 40%（= red-ball-push TURN_STEP_POWER 40） */
...
#define BALL_PUSH_FORWARD_MS      1200
#define BALL_PUSH_PAUSE_MS        500
#define BALL_PUSH_RETURN_MS       1230

/* ================= 击绿球阶段参数（整体移植 green-ball-push） =================
 * 与击红球逻辑完全分离：红球 RETURN 完成后才使能绿球检测并进入本状态机。
 * 判绿：G>75 且 绿度=G-(R+B)/2>30；搜索向右；过中线反向步长取 2/3。
 */
#define GREEN_MAP_CENTER_X        40
#define GREEN_DETECT_PIXELS       3      /* 绿点触发数（按 green-ball-push 代码值） */
#define GREEN_CONFIRM_FRAMES      3
#define GREEN_CENTER_DEADBAND_PX  5
#define GREEN_CENTER_CONFIRM_FRAMES 3
#define GREEN_ALIGN_LOST_FRAMES   3

#define GREEN_SEARCH_STEP_MS      40
#define GREEN_ALIGN_STEP_INIT_MS  50
#define GREEN_ALIGN_STEP_MIN_MS   20
#define GREEN_OVERSHOOT_NUM       2      /* 过中线后步长 = 原步长 × 2/3 */
#define GREEN_OVERSHOOT_DEN       3
#define GREEN_TURN_BRAKE_MS       90
#define GREEN_TURN_SETTLE_MS      200

/* 0..255 换算：70% = 179，58% = 148，62% = 158 */
#define GREEN_TURN_SPEED          204    /* 转向脉冲功率 80%（0..255） */
#define GREEN_PUSH_LF_SPEED       148
#define GREEN_PUSH_RF_SPEED       158
#define GREEN_PUSH_FORWARD_MS     1200
#define GREEN_PUSH_PAUSE_MS       500
#define GREEN_PUSH_RETURN_MS      1230
...
static int ball_align_step_ms = BALL_SEARCH_STEP_MS;
static int ball_prev_err_side = 0;         /* 上一次红球偏差方向：-1 左 / +1 右 / 0 未知 */
static int ball_dir = -1;                  /* 当前转向方向：-1 左 / +1 右，默认左转 */
...
/* ================= 击绿球子阶段（与击红球完全分离） ================= */
static bool red_phase_done = false;        /* 红球流程是否已完整结束 */
...
static green_phase_state_t green_state = GREEN_OFF;
static uint32_t green_state_start_ms = 0;
static int green_detect_conf = 0;
static int green_center_conf = 0;
static int green_lost_frames = 0;
static int green_align_step_ms = GREEN_ALIGN_STEP_INIT_MS;
static int green_prev_err_side = 0;
static bool green_search_need_step = false;
static bool green_turn_active = false;
static bool green_brake_active = false;
static uint32_t green_turn_until_ms = 0;
static uint32_t green_brake_until_ms = 0;
static uint32_t green_settle_until_ms = 0;
static uint32_t green_last_frame_id = 0;

static void green_reset_all(void);
static void start_green_phase(uint32_t now);
static void handle_green_ball_phase(uint32_t now);
...
static bool handle_avoidance(uint32_t now, uint8_t bits)
{
    ...
    case AVOID_STRAFE_BACK:
        apply_strafe(1);
        /* 至少右移 STRAFE_BACK_MIN_MS；
         * 400~1400ms 之间重新看到黑线：立即停止右移进入直行段 */
...
static void ball_reset_all(void)
{
    ...
    ball_center_conf = 0;
    ball_lost_frames = 0;
    ball_align_step_ms = BALL_SEARCH_STEP_MS;
    ...
    /* 击绿球子阶段：整体复位，且红球完成标志清零 */
    red_phase_done = false;
    green_reset_all();
    green_vision_set_enabled(false);
}
...
static void handle_ball_phase(uint32_t now)
{
            ...
            if (!red_detected) {
                ...
                if (dist <= BALL_CENTER_DEADBAND_PX) {
                    ...
                    if (ball_prev_err_side != 0 &&
                        ...
                        printf("BALL: center crossed; reverse %s half step=%dms\n",
                               ball_dir < 0 ? "LEFT" : "RIGHT",
                               ball_align_step_ms);
                    }
                    ball_prev_err_side = err_side;
                    ball_start_turn(now, ball_dir, ball_align_step_ms);
                    printf("BALL: scan step %s step=%dms\n",
                           ball_dir < 0 ? "LEFT" : "RIGHT",
                           ball_align_step_ms);
                }
            }
    ...
    if (ball_state == BALL_SEARCH && ball_search_need_step &&
    ...
    } else if (ball_turn_active || ball_brake_active) {
        /* 保持搜索/对准脉冲或刹车输出，直到上方计时器结束。
         * 旧代码在这里直接落入默认分支并停车，导致脉冲只剩约 20ms。 */
        ...
        if (now - ball_state_start_ms >= BALL_PUSH_RETURN_MS) {
            ...
            printf("BALL: red push done -> switch to GREEN phase\n");
            start_green_phase(now);
        }
    } else {
        /* BALL_DONE / BALL_OFF */
        stop_all_motors();
...
static void green_reset_all(void)
{
    ...
    green_state_start_ms = 0;
    green_detect_conf = 0;
    green_center_conf = 0;
    green_lost_frames = 0;
    green_align_step_ms = GREEN_ALIGN_STEP_INIT_MS;
    green_prev_err_side = 0;
    green_search_need_step = false;
    green_turn_active = false;
    green_brake_active = false;
    green_turn_until_ms = 0;
    green_brake_until_ms = 0;
    green_settle_until_ms = 0;
    green_last_frame_id = 0;
}
...
static void start_green_phase(uint32_t now)
{
    red_phase_done = true;
    green_reset_all();
    green_state = GREEN_SEARCH;
    green_search_need_step = true;
    green_settle_until_ms = now;   /* 进入即允许读帧/发第一个脉冲 */

    /* 红球视觉关闭，绿球视觉开启：两套检测互不干扰 */
    ball_vision_set_enabled(false);
    green_vision_set_enabled(true);
    stop_all_motors();
    printf("GREEN: phase started - stepped RIGHT search (step=%dms)\n",
           GREEN_SEARCH_STEP_MS);
}

static void handle_green_ball_phase(uint32_t now)
{
    green_vision_t v;
    bool got = green_vision_get(&v);

    /* 每个转向脉冲：到时 -> 短刹 -> 刹停 -> 稳定等待再读帧 */
    if (green_turn_active && now >= green_turn_until_ms) {
        motor_brake_all();
        green_turn_active = false;
        green_brake_active = true;
        green_brake_until_ms = now + GREEN_TURN_BRAKE_MS;
    }
    if (green_brake_active && now >= green_brake_until_ms) {
        stop_all_motors();
        green_brake_active = false;
        green_settle_until_ms = now + GREEN_TURN_SETTLE_MS;
    }

    bool frame_new = got && (v.frame_id != green_last_frame_id);
    if (frame_new) {
        green_last_frame_id = v.frame_id;
    }

    /* 找球 / 对准只在：无脉冲、无刹车、已稳定、且视觉出了新帧时判断 */
    if ((green_state == GREEN_SEARCH || green_state == GREEN_ALIGN) &&
        !green_turn_active && !green_brake_active &&
        now >= green_settle_until_ms && frame_new) {

        bool green_detected = got && v.green_pixels >= GREEN_DETECT_PIXELS;

        if (green_state == GREEN_SEARCH) {
            green_detect_conf = green_detected ? green_detect_conf + 1 : 0;
            green_search_need_step = !green_detected;
            if (green_detect_conf >= GREEN_CONFIRM_FRAMES) {
                green_state = GREEN_ALIGN;
                green_center_conf = 0;
                green_lost_frames = 0;
                green_align_step_ms = GREEN_ALIGN_STEP_INIT_MS;
                green_prev_err_side = 0;
                green_search_need_step = false;
                printf("GREEN: detected pix=%d cx=%d -> ALIGN\n",
                       v.green_pixels, v.green_cx);
            }
        }

        if (green_state == GREEN_ALIGN) {
            if (!green_detected) {
                ++green_lost_frames;
                green_center_conf = 0;
                if (green_lost_frames >= GREEN_ALIGN_LOST_FRAMES) {
                    green_state = GREEN_SEARCH;
                    green_detect_conf = 0;
                    green_align_step_ms = GREEN_ALIGN_STEP_INIT_MS;
                    green_prev_err_side = 0;
                    green_search_need_step = true;
                    printf("GREEN: lost while aligning -> SEARCH\n");
                }
            } else {
                green_lost_frames = 0;
                int err = v.green_cx - GREEN_MAP_CENTER_X;
                int dist = (err < 0) ? -err : err;
                if (dist <= GREEN_CENTER_DEADBAND_PX) {
                    stop_all_motors();
                    ++green_center_conf;
                    if (green_center_conf >= GREEN_CENTER_CONFIRM_FRAMES) {
                        green_state = GREEN_PUSH;
                        green_state_start_ms = now;
                        green_prev_err_side = 0;
                        printf("*** GREEN: centered cx=%d pix=%d -> PUSH ***\n",
                               v.green_cx, v.green_pixels);
                    }
                } else {
                    green_center_conf = 0;
                    int err_side = (err < 0) ? -1 : 1;
                    if (green_prev_err_side != 0 &&
                        err_side != green_prev_err_side) {
                        /* 过中线：反向并把步长变为原来的 2/3 */
                        green_align_step_ms =
                            (green_align_step_ms * GREEN_OVERSHOOT_NUM) /
                            GREEN_OVERSHOOT_DEN;
                        if (green_align_step_ms < GREEN_ALIGN_STEP_MIN_MS) {
                            green_align_step_ms = GREEN_ALIGN_STEP_MIN_MS;
                        }
                        printf("GREEN: center crossed; reverse 2/3 step=%dms\n",
                               green_align_step_ms);
                    }
                    green_prev_err_side = err_side;
                    apply_turn_dir(err_side, GREEN_TURN_SPEED);
                    green_turn_active = true;
                    green_turn_until_ms = now + (uint32_t)green_align_step_ms;
                    printf("GREEN: align step err=%d %s step=%dms\n",
    ...
    if (green_state == GREEN_SEARCH && green_search_need_step &&
        !green_turn_active && !green_brake_active &&
        now >= green_settle_until_ms) {
        /* 找不到目标：向右脉冲搜索 */
        apply_turn_dir(1, GREEN_TURN_SPEED);
        green_turn_active = true;
        green_turn_until_ms = now + GREEN_SEARCH_STEP_MS;
        green_search_need_step = false;
        printf("GREEN: search step RIGHT %dms\n", GREEN_SEARCH_STEP_MS);
    } else if (green_turn_active || green_brake_active) {
        /* 保持绿球搜索/对准脉冲或刹车输出，直到计时结束；
         * 不能被末尾的默认 stop_all_motors() 立即覆盖。 */
    } else if (green_state == GREEN_PUSH) {
        motor_set_lf(GREEN_PUSH_LF_SPEED);
        motor_set_rf(GREEN_PUSH_RF_SPEED);
        motor_set_rear(0);
        if (now - green_state_start_ms >= GREEN_PUSH_FORWARD_MS) {
            stop_all_motors();
            green_state = GREEN_PAUSE;
            green_state_start_ms = now;
            printf("GREEN: push forward done, pause\n");
        }
    } else if (green_state == GREEN_PAUSE) {
        stop_all_motors();
        if (now - green_state_start_ms >= GREEN_PUSH_PAUSE_MS) {
            green_state = GREEN_RETURN;
            green_state_start_ms = now;
            printf("GREEN: pause done, return backward\n");
        }
    } else if (green_state == GREEN_RETURN) {
        motor_set_lf(-GREEN_PUSH_LF_SPEED);
        motor_set_rf(-GREEN_PUSH_RF_SPEED);
        motor_set_rear(0);
        if (now - green_state_start_ms >= GREEN_PUSH_RETURN_MS) {
            stop_all_motors();
            green_state = GREEN_DONE;
            printf("GREEN: return done, all ball phases complete\n");
        }
    } else {
        /* GREEN_DONE / GREEN_OFF */
        stop_all_motors();
    }
}
...
{
    return ball_phase && red_phase_done &&
           green_state != GREEN_OFF;
}
