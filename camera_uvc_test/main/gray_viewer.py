...
FRAME_BYTES = W * H
FRAME_TOTAL = 4 + 4 + 2 + 3 + FRAME_BYTES + 2  # magic + w/h + th + near/far/corner + data + crc16
...
class GrayViewer:
    def __init__(self, port_name, baud):
        ...
        self.tri_bin = self.canvas_bin.create_polygon(tri, outline="yellow",
                                                      fill="")
        # 4 路虚拟红外的分区边界（80x60 流坐标 x=25/33/40/48/55，
        # 即检测图 bit3[50,65) bit2[65,80) bit1[80,95) bit0[95,110) 的边界）
        for zx in (25, 33, 40, 48, 55):
            x = zx * SCALE
        ...
        self.far_label = tk.Label(self.root, text="far=0b0000 corner=0",
                                  font=("Menlo", 12))
        self.far_label.pack()
    ...
    def update_zones(self):
        ...
        self.far_label.config(
            text="near=0b%d%d%d%d  far=0b%d%d%d%d  corner=%d" % (
        ...
        try:
            while self.running:
                ...
                while True:
                    ...
                    self.corner = buf[idx + 12]
                    payload = buf[idx + 13:idx + 13 + FRAME_BYTES]
                    crc = struct.unpack("<H", buf[idx + 13 + FRAME_BYTES:
                                                   idx + FRAME_TOTAL])[0]
