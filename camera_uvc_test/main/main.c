...
static void camera_proc_task(void *arg)
{
    ...
    while (1) {
        ...
        int64_t min_gap_us = ball_vision_is_enabled()
                             ? (100 * 1000)   /* 击球：与 red-ball-push 10fps 一致 */
        ...
        /* 击球阶段：循线黑线检测完全退出，只做红球检测（red-ball-push 思路） */
...
static void stream_task(void *arg)
{
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "trying stream: %s %ux%u",
