...
# Keep the task terminal as the viewer's status panel. Open the browser from
# macOS explicitly, rather than relying on Python's browser detection in a
# non-interactive VS Code task shell.
"$VENV_DIR/bin/python" "$PROJECT_DIR/tools/gray_viewer.py" "$SERIAL_PORT" --no-browser &
VIEWER_PID=$!
