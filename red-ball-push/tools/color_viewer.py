...
<title>ESP32-S3 摄像头红球识别</title><style>
...
p{color:#9aa4b2;font-size:13px}</style></head><body><h2>ESP32-S3 摄像头实时红球识别</h2>
<div id="status">等待图像帧…</div><div class="controls"><label>红度阈值 <input id="red" type="range" min="0" max="160" value="71"><output id="redValue">71</output></label></div><div class="views"><div class="view"><span>原始彩色图</span><canvas id="image"></canvas></div><div class="view"><span>红色 / 非红色 二值图</span><canvas id="classify"></canvas></div></div>
<p>只识别红色：红色显示为红，其余所有像素显示为白色。当前默认阈值与小车识别阈值一致。</p>
<script>const canvas=document.getElementById('image'),ctx=canvas.getContext('2d'),classCanvas=document.getElementById('classify'),classCtx=classCanvas.getContext('2d'),status=document.getElementById('status'),red=document.getElementById('red');let last=-1,lastPixels=null,lastWidth=0,lastHeight=0;
