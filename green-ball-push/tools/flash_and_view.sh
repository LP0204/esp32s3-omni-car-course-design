#!/bin/zsh
# Flash the green-ball controller, then immediately open its 480p preview.
# Run through the VS Code task "Green Ball: Flash and Open Preview".
...
"$VENV_DIR/bin/python" "$PROJECT_DIR/tools/green_viewer.py" "$SERIAL_PORT" &
VIEWER_PID=$!
...
for attempt in {1..50}; do
    if ! kill -0 "$VIEWER_PID" 2>/dev/null; then
        print "新查看器启动失败。请在旧查看器终端按 Ctrl+C 后重新运行，避免打开旧服务。"
        wait "$VIEWER_PID"
        exit 1
    fi
